...
class Castep(CMakePackage):
    ...
    variant('build_type', default='fast', description='CASTEP build type', values=('debug', 'intermediate', 'fast'))
    ...
    #variant("mace", description="Add support for MACE", default=False) # This doesn't seem to work...
    ...
    #depends_on("py-torch", type=("build","link"), when="+mace")
    ...
    def test_castep_si2(self):
        ...
        test_dir = join_path(self.test_suite.current_test_cache_dir, "Test", "Electronic", "Si2-den")
    ...
    def test_castepconv_wrapper(self):
        """Check that the python script wrapper installed correctly and passes arguments for castepconv.py"""
        if self.spec.satisfies("+utilities"):
